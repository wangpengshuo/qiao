...
from utils.distance import object_width
...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3, length=None):
    # Plots one bounding box on image 'im' using OpenCV
    assert im.data.contiguous, 'Image not contiguous. Apply np.ascontiguousarray(im) to plot_on_box() input image.'
    tl = line_thickness or round(0.002 * (im.shape[0] + im.shape[1]) / 2) + 1  # line/font thickness
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    h = (int(x[3]) - int(x[1])) / 18.95  # 框的高
    t = int(x[3]) - int(x[1])
    marker = find_marker(im)
    P = marker[1][0] / 18.95
    # # dis_m = 1.00
    # if name == 'crack0':  # 根据标签名称调用不同函数计算距离
    dis_m = round(object_width(length, h), 2)
    label = f'{label}  {dis_m}cm'
    print('label:', label)
